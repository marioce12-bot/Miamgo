...
export async function POST(request) {
  ...
  if (metadata.type === "subscription" && ["transaction.approved", "transaction.transferred"].includes(name)) { const restaurantRef = db.collection("restaurants").doc(String(metadata.restaurantId)); const current = await restaurantRef.get(); const previousExpiry = current.data()?.subscriptionStatus === "active" && current.data()?.subscriptionExpiresAt?.toDate?.() > new Date() ? current.data().subscriptionExpiresAt.toDate() : new Date(); const expiry = new Date(previousExpiry); expiry.setDate(expiry.getDate() + 30); await restaurantRef.set({ plan: metadata.plan, subscriptionStatus: "active", subscriptionExpiresAt: expiry, subscriptionTransactionId: String(data.id || ""), updatedAt: new Date() }, { merge: true }); return NextResponse.json({ received: true, subscription: "active" }); }
