...
export async function POST(request) {
  ...
  if (!secret || !signature) return NextResponse.json({ error: "Signature webhook absente." }, { status: 401 });
  const expectedSignature = crypto.createHmac("sha256", secret).update(rawBody).digest("hex");
  if (signature.length !== expectedSignature.length || !crypto.timingSafeEqual(Buffer.from(signature), Buffer.from(expectedSignature))) return NextResponse.json({ error: "Signature invalide." }, { status: 401 });
  let event; try { event = JSON.parse(rawBody); } catch { return NextResponse.json({ error: "Payload invalide." }, { status: 400 }); }
  const name = event?.name || event?.event || event?.type || "unknown"; const data = event?.data || event?.object || event?.transaction || event?.payout || {}; const eventId = String(event?.id || data?.id || crypto.createHash("sha256").update(rawBody).digest("hex")); const metadata = data?.custom_metadata || data?.metadata || {}; const orderId = metadata?.orderId || metadata?.order_id;
  const db = getAdminDb(); const eventRef = db.collection("fedapayEvents").doc(eventId); const already = await eventRef.get(); if (already.exists) return NextResponse.json({ received: true, duplicate: true }); await eventRef.set({ name, orderId: orderId || null, restaurantId: metadata.restaurantId || null, amount: Number(data.amount || metadata.amount || 0), paymentStatus: name === "transaction.approved" || name === "transaction.transferred" ? "paid" : "failed", receivedAt: new Date() });
  if (metadata.type === "subscription" && ["transaction.approved", "transaction.transferred"].includes(name)) { const restaurantRef = db.collection("restaurants").doc(String(metadata.restaurantId)); const current = await restaurantRef.get(); const previousExpiry = current.data()?.subscriptionStatus === "active" && current.data()?.subscriptionExpiresAt?.toDate?.() > new Date() ? current.data().subscriptionExpiresAt.toDate() : new Date(); const expiry = new Date(previousExpiry); expiry.setDate(expiry.getDate() + 30); await restaurantRef.set({ plan: metadata.plan, subscriptionStatus: "active", subscriptionExpiresAt: expiry, subscriptionTransactionId: String(data.id || ""), updatedAt: new Date() }, { merge: true }); await db.collection("subscriptions").doc(String(data.id || eventId)).set({ restaurantId: String(metadata.restaurantId), ownerId: metadata.ownerId || null, plan: metadata.plan, amount: Number(data.amount || metadata.amount || 0), status: "paid", paymentStatus: "paid", transactionId: String(data.id || eventId), paidAt: new Date(), updatedAt: new Date() }, { merge: true }); return NextResponse.json({ received: true, subscription: "active" }); }
