...
export default function LoginPage() {
  ...
  async function login(event) { event.preventDefault(); setLoading(true); setError(""); const form = new FormData(event.currentTarget); try { const credential = await signInWithEmailAndPassword(auth, form.get("email"), form.get("password")); const profile = await getUserProfile(credential.user.uid); const restaurant = profile?.role === "restaurant_owner" ? true : await getOwnedRestaurant(credential.user.uid); router.push(restaurant ? "/espace-resto" : profile?.role === "driver" ? "/espace-livreur" : "/accueil"); } catch { setError("Connexion impossible. Vérifiez votre adresse e-mail et votre mot de passe."); } finally { setLoading(false); } }
