...
export default function CheckoutPage() {
  ...
  async function beginOrder() {
    ...
    try {
      ...
      const paymentResponse = await fetch("/api/fedapay/create-transaction", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ orderId: order.id, foodSubtotal: 3000, deliveryFee: 0, courierShare: 0, customerEmail: user.email, customerCountry: "bj" }) });
      const payment = await paymentResponse.json();
      if (!paymentResponse.ok) throw new Error(payment.error || "FedaPay a refusé la transaction.");
      setOrderStatus(`Commande ${order.serialNumber} créée. Transaction FedaPay ${payment.transaction?.id || payment.transaction?.data?.id || "en attente"}.`);
