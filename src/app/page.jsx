    ...
    <main className="app-shell">
      <header className="topbar">
        ...
        <nav className="desktop-actions">
          ...
          {user ? <button className="profile-button" onClick={() => signOut(auth)}><CircleUserRound size={22} /> {user.email?.split("@")[0] || "Mon profil"}</button> : <button className="login-button" onClick={() => setShowLogin(true)}>Se connecter</button>}
        </nav>
      </header>
      ...
      <div className="layout">
        ...
        <section className="feed" id="feed">
          <div className="feed-intro"><div><p className="eyebrow">Autour de vous</p><h1>Qu&apos;est-ce qui vous fait envie?</h1></div><button className="filter-button"><Menu size={18} /> Filtrer</button></div>
          <div className="story-row">
            <button className="story create-story" onClick={() => requireAuth(() => {})}><span><Plus size={20} /></span><small>Partager une envie</small></button>
            {restaurants.map(([name, rating, distance, initials, color]) => <button className="story" key={name}><span style={{ backgroundColor: color }}>{initials}</span><small>{name}</small><em>{rating} · {distance}</em></button>)}
          </div>
          ...
          {visiblePosts.map((post) => {
            ...
            return <article className="post-card" key={post.id}>
              ...
              <div className="post-head"><span className="restaurant-avatar" style={{ backgroundColor: post.color }}>{post.avatar}</span><div><strong>{post.restaurant}</strong><p>{post.handle} <span>·</span> {post.time}</p><small><MapPin size={12} />{post.location}</small></div><button className="more">•••</button></div>
              ...
              <div className="order-row"><div><span>Plat du jour</span><strong>{post.price}</strong></div><button onClick={() => requireAuth(() => addToCart(post))}><Plus size={18} />Ajouter au panier</button><button className="order-now" onClick={() => requireAuth(() => addToCart(post))}>Commander</button></div>
            </article>;
          })}
