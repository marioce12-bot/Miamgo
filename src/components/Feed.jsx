...
function StoryViewer({ stories, index, user, onClose }) {
  const story = stories[index]; const [views, setViews] = useState(0); const mediaSource = story?.mediaUrl || story?.url || story?.imageUrl || story?.media?.url || "";
  ...
  return <div className="story-viewer" role="dialog" aria-label="Story"><div className="story-progress-multi">{stories.map((item, itemIndex) => <span className={itemIndex < index ? "done" : itemIndex === index ? "current" : ""} key={item.id}/>)}</div><button className="story-viewer-close" onClick={() => onClose(null)}>×</button><div className="story-viewer-media">{mediaSource ? story.mediaType === "video" ? <video src={mediaSource} autoPlay playsInline onEnded={() => onClose(index + 1 < stories.length ? index + 1 : null)} /> : <img src={mediaSource} alt={`Story de ${story.authorName || "Utilisateur"}`} /> : <p className="story-media-error">Média de cette story indisponible.</p>}<button className="story-touch-zone story-touch-left" onClick={() => index > 0 && onClose(index - 1)} aria-label="Story précédente"/><button className="story-touch-zone story-touch-right" onClick={() => onClose(index + 1 < stories.length ? index + 1 : null)} aria-label="Story suivante"/></div><strong>{story.authorName || "Utilisateur"}</strong>{user?.uid === story.ownerId && <span className="story-view-count">◉ {views}</span>}</div>;
