...
function StoryViewer({ stories, index, user, onClose }) {
  ...
  return <div className="story-viewer" role="dialog" aria-label="Story"><div className="story-progress-multi">{stories.map((item, itemIndex) => <span className={itemIndex < index ? "done" : itemIndex === index ? "current" : ""} key={item.id}/>)}</div><button className="story-viewer-close" onClick={() => onClose(null)}>×</button><div className="story-viewer-media">{story.mediaType === "video" ? <video src={story.mediaUrl} autoPlay playsInline onEnded={() => onClose(index + 1 < stories.length ? index + 1 : null)} /> : <img src={story.mediaUrl} alt={`Story de ${story.authorName || "Utilisateur"}`} />}<button className="story-touch-zone story-touch-left" onClick={() => index > 0 && onClose(index - 1)} aria-label="Story précédente"/><button className="story-touch-zone story-touch-right" onClick={() => onClose(index + 1 < stories.length ? index + 1 : null)} aria-label="Story suivante"/></div><strong>{story.authorName || "Utilisateur"}</strong>{user?.uid === story.ownerId && <span className="story-view-count">◉ {views}</span>}</div>;
