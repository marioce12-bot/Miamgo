...
function StoryViewer({ stories, index, user, onClose }) {
  const story = stories[index]; const [views, setViews] = useState(0); const [mediaError, setMediaError] = useState(false); const mediaSource = story?.mediaUrl || story?.url || story?.imageUrl || story?.media?.url || "";
  ...
  useEffect(() => setMediaError(false), [story?.id]);
  if (!story) return null;
  return <div className="story-viewer" role="dialog" aria-label="Story"><div className="story-progress-multi">{stories.map((item, itemIndex) => <span className={itemIndex < index ? "done" : itemIndex === index ? "current" : ""} key={item.id}/>)}</div><button className="story-viewer-close" onClick={() => onClose(null)}>×</button><div className="story-viewer-media">{mediaSource && !mediaError ? story.mediaType === "video" ? <video src={mediaSource} autoPlay playsInline onError={() => setMediaError(true)} onEnded={() => onClose(index + 1 < stories.length ? index + 1 : null)} /> : <img src={mediaSource} alt={`Story de ${story.authorName || "Utilisateur"}`} onError={() => setMediaError(true)} /> : <div className="story-media-error"><span>MIAMGO</span><strong>{story.authorName || "Utilisateur"}</strong><small>Cette story n&apos;est plus disponible.</small></div>}<button className="story-touch-zone story-touch-left" onClick={() => index > 0 && onClose(index - 1)} aria-label="Story précédente"/><button className="story-touch-zone story-touch-right" onClick={() => onClose(index + 1 < stories.length ? index + 1 : null)} aria-label="Story suivante"/></div><strong>{story.authorName || "Utilisateur"}</strong>{user?.uid === story.ownerId && <span className="story-view-count">◉ {views}</span>}</div>;
    ...
    <main className={`app-shell ${roleReady ? "" : "role-feed-loading"}`}>{storyViewer !== null && <StoryViewer stories={storyGroups[storyViewer.group] || []} index={storyViewer.index} user={user} onClose={(next) => next === null ? setStoryViewer(null) : setStoryViewer({ group: storyViewer.group, index: next })}/>}
      ...
      <div className="layout">
        ...
        <section className="feed" id="feed">
          <div className="feed-intro"><div><h1>{role === "restaurant_owner" ? t("feed") : t("craving")}</h1></div>{role !== "restaurant_owner" && <button className="filter-button" onClick={() => document.querySelector(".search-box input")?.focus()}><Menu size={18} /> {t("filter")}</button>}</div>
          ...
          {visiblePosts.map((post) => {
            ...
            return <article className="post-card" id={`post-${post.id}`} key={post.id}>
              ...
              <div className="post-head"><button className="restaurant-avatar" style={{ backgroundColor: post.color }} onClick={() => router.push(`/restaurant/${post.restaurantId || ""}`)} aria-label={`Voir ${post.restaurant}`}>{post.avatarImage ? <img src={post.avatarImage} alt="" /> : post.avatar}</button><div><button className="restaurant-name" onClick={() => router.push(`/restaurant/${post.restaurantId || ""}`)}>{post.restaurant}</button><p>{post.handle} <span>·</span> {post.time}</p><small><MapPin size={12} />{post.location}</small></div><button className="more" onClick={sharePost}><Share2 size={18} /></button></div>
